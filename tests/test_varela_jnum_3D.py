...
def test_error_estimates_matching_grid_cell_size_0125(material_constants: dict) -> None:
    """Test error estimates for matching grid (h=0.125)."""
...
def test_full_non_matching_side_grids(
    ...
    frac_faces = sd_matrix.tags["fracture_faces"]
    np.testing.assert_array_almost_equal(
        sd_matrix.face_areas[frac_faces], left_intb_h,
    )

    # Assert cell sizes of fracture cells
    sd_fracture = mdg.subdomains()[1]
    np.testing.assert_array_almost_equal(
        sd_fracture.cell_volumes, frac_h,
    )
    ...
    for side in intf.sides:
        if side == MortarSides.LEFT_SIDE:
            np.testing.assert_array_almost_equal(
                intf.side_grids[side].cell_volumes, left_intf_h,
            )
        elif side == MortarSides.RIGHT_SIDE:
            np.testing.assert_array_almost_equal(
                intf.side_grids[side].cell_volumes, right_intf_h,
            )
