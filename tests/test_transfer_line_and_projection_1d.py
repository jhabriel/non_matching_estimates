...
def test_constant_exact(g_src, g_tgt):
    ...
    np.testing.assert_allclose(u_tr, 3.14, rtol=0, atol=1e-13)
...
def test_linear_exact(g_src, g_tgt):
    ...
    # full pipeline TL ➜ SZ ➜ target
    C_tgt = project_p1_1d(g_src, g_tgt, C_src)
    xt = g_tgt.nodes[0, :]
    mids = 0.5 * (xt[:-1] + xt[1:])
    u_tgt = C_tgt[:,0]*mids + C_tgt[:,1]
    np.testing.assert_allclose(u_tgt, a*mids + b, rtol=0, atol=1e-13)
...
def test_identity_on_matching(g_src):
    ...
    C_tgt = project_p1_1d(g_src, g_src, C_src)
    np.testing.assert_allclose(C_tgt, C_src, rtol=0, atol=1e-13)

def test_sz_stability_random(g_src, g_tgt):
    # random broken P1 on source → TL→SZ; ensure it returns finite values and no shape issues
