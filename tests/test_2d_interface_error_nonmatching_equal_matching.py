...
def set_constant_p1_on_grid(g: pp.Grid, c: float) -> np.ndarray:
    ...
    if g.dim == 2:
        ...
        return np.column_stack(
            [np.zeros(nc), np.zeros(nc), np.zeros(nc), np.full(nc, c)]
        )
    elif g.dim == 1:
...
def _base_params(
    material_constants,
    *,
    refine_mortar: bool = False,
    refine_fracture: bool = False,
) -> dict:
    """Construct params consistent with the new Geometry logic.

    non_matching is True iff we actually refine fracture or mortar.
    """
    non_matching = refine_mortar or refine_fracture
    return {
        "grid_type": "simplex",
...
@pytest.mark.parametrize(
    "refine_mortar,refine_fracture,lam_mode,c_high,c_low",
    [
        # matching cases (no refinement: purely matching interface)
...
def test_constants_matching_vs_nonmatching(
    ...
    else:  # "zero" with c_high==c_low for these tests
        expected = np.zeros_like(dN)
...
def test_interface_diffusive_matching_equals_nonmatching_on_matching_grid(
    material_constants,
):
    # Matching config
    params = {
