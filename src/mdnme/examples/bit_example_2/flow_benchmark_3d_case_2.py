...
Note:
    Two classes are provided `FlowBenchmark3dCase2ModelCond` and
    `FlowBenchmark3dCase2ModelBlock`. The former is the first variant of the
    benchmark where fractures are conductive, whereas the latter is the second
    variant where fractures are blocking. Both classes admit the parameter keyword
...
class PermeabilitySpecification(Permeability):
    """Mixin to set up permeability values in the mixed-dimensional domain"""

    def permeability(self, subdomains: list[pp.Grid]) -> pp.ad.Operator:
        """Heterogeneous matrix permeability specification."""

        def low_perm_zones(sd: pp.Grid) -> np.ndarray:
            """Boolean mask for lower permeability zones of the 3d subdomain."""

            if sd.dim < 3:
                return np.zeros(sd.num_cells, dtype=bool)

            zone_0 = np.logical_and(sd.cell_centers[0, :] > 0.5,
                                    sd.cell_centers[1, :] < 0.5)

            zone_1 = np.logical_and.reduce(
                tuple(
                    [
                        sd.cell_centers[0, :] > 0.75,
                        sd.cell_centers[1, :] > 0.5,
                        sd.cell_centers[1, :] < 0.75,
                        sd.cell_centers[2, :] > 0.5,
                    ]
                )
            )
            ...
            zone_2 = np.logical_and.reduce(
                tuple(
                    [
                        sd.cell_centers[0, :] > 0.625,
                        sd.cell_centers[0, :] < 0.75,
                        sd.cell_centers[1, :] > 0.5,
                        sd.cell_centers[1, :] < 0.625,
                        sd.cell_centers[2, :] > 0.5,
                        sd.cell_centers[2, :] < 0.75,
                    ]
                )
            )
        ...
        for sd in subdomains:
            ...
            if sd.dim == 3:
                # Unit permeability except in the low permeability zones
                kxx[low_perm_zones(sd)] = 1e-1
                vals.append(kxx)
            else:
                vals.append(kxx)  # just a placeholder, won't be use
        if len(subdomains) > 0:
...
class BoundaryConditions(pp.PorePyModel):
    ...
    def bc_type_darcy_flux(self, sd: pp.Grid) -> pp.BoundaryCondition:
        """Assign Dirichlet the outlet boundary"""
        tol = 1e-8
        ...
        if b_faces != 0:
            ...
            # Outlet faces correspond to Dirichlet boundary conditions.
            # The rest are set as Neumann by default.
    ...
    def bc_values_darcy_flux(self, bg: pp.BoundaryGrid) -> np.ndarray:
        ...
        cc = bg.cell_centers
        nc = bg.num_cells
        vols = bg.cell_volumes

        # Get inlet faces.
        inlet_faces = np.logical_and.reduce(
            tuple(cc[i, :] < 0.25 + tol for i in range(3))
        )
        val = self.units.convert_units(-1, "m * s^-1")
        values = np.zeros(nc)
        values[inlet_faces] = val * vols[inlet_faces]
        return values

    def bc_values_pressure(self, bg: pp.BoundaryGrid) -> np.ndarray:
        """Assign unitary pressure at the outlet boundary:

            \partial\Omega_outlet = \{x \in \partial_\Omega : x_1, x_2, x_3 > 0.875\}
        ...
        outlet_faces = np.logical_and.reduce(
            tuple(cc[i, :] > 0.875 + tol for i in range(3))
        )
...
class FlowBenchmark3dCase2Model(  # type:ignore[misc]
    ...
    To set up the case with conductive fractures, use `solid_constants_conductive`.
    To set up the case with blocking fractures, use `solid_constants_blocking`.
