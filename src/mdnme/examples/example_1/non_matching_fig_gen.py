...
def export_transfer_grids(mdg, side: str, out_prefix: str = "tg") -> None:
    ...
    tg_ibg2msg  = TransferGrid(
        g_source=ibg_side, g_target=sidegrid,
        rotation_matrix=R_side, tol=1e-8,
        name=f"{out_prefix}_ibg2{side}"
    )
    tg_frac2msg = TransferGrid(
        g_source=sd_frac,  g_target=sidegrid,
        rotation_matrix=R_side, tol=1e-8,
...
def _pick_side_enum(intf: pp.MortarGrid, which: str) -> int:
    ...
    for enum, sg in intf.side_grids.items():
        ...
        if n_phys[0] < 0: left_enum = enum
        else:             right_enum = enum
    if which == "left":
...
def _embed_transfer_nodes_to_3d(g2d_nodes: np.ndarray, sidegrid: pp.Grid, x_plane: float) -> np.ndarray:
    """
...
def _add_transfer_wire(ax, tg, sidegrid: pp.Grid, *,
                       x_plane: float, edge="#333", lw=0.8, face_alpha=0.0):
    """
...
# -------------- main --------------
def build_mdg(h: float, *, non_matching: bool, translation=None) -> pp.MixedDimensionalGrid:
...
def make_figure_with_transfers(mdg: pp.MixedDimensionalGrid, side: str, outfile: str):
    ...
    try:
        ...
        _add_transfer_wire(ax, tg_ibg2msg, sidegrid,
                           x_plane=x_tg_ibg,
                           edge=TG_EDGE_COLORS["ibg2msg"],
                           lw=TG_EDGE_WIDTHS["ibg2msg"],
                           face_alpha=TG_FACE_ALPHA)
        _add_transfer_wire(ax, tg_frac2msg, sidegrid,
                           x_plane=x_tg_frac,
