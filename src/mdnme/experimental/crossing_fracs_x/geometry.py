...
class TwoCrossingFractures:
    ...
    def set_fractures(self) -> None:
        ...
        if self.grid_type() == "simplex":
            ghost_frac_1 = pp.LineFracture(np.array([[0.00, 1.00], [0.25, 0.25]]))
            ghost_frac_2 = pp.LineFracture(np.array([[0.00, 1.00], [0.75, 0.75]]))
            ghost_frac_3 = pp.LineFracture(np.array([[0.25, 0.25], [0.25, 0.75]]))
            ghost_frac_4 = pp.LineFracture(np.array([[0.75, 0.75], [0.25, 0.75]]))
            self._fractures = [
                f1,
                f2,
                f3,
    ...
    def meshing_kwargs(self) -> dict:
        ...
        kw_args = {}
        if self.grid_type() == "simplex":
            kw_args = {"constraints": np.array([4, 5, 6, 7])}
        return kw_args
