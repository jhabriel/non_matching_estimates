"""
This is an implementation of the exact solution to the numerical example 1 from the
paper. The exact solutions are obtained using SymPy.
"""
...
class SaveData:
    ...
    # Approximated pressures, fluxes, and interface fluxes.
    approx_p_2d_0: np.ndarray
    approx_p_1d_0: np.ndarray
    approx_p_1d_1: np.ndarray
    approx_p_1d_2: np.ndarray
    approx_p_1d_3: np.ndarray
    approx_p_0d_0: np.ndarray
    approx_u_2d_0: np.ndarray
    approx_u_1d_0: np.ndarray
    approx_u_1d_1: np.ndarray
    approx_u_1d_2: np.ndarray
    approx_u_1d_3: np.ndarray
    approx_lambda_1d_0: np.ndarray
    approx_lambda_1d_1: np.ndarray
    approx_lambda_1d_2: np.ndarray
    approx_lambda_1d_3: np.ndarray
    approx_lambda_0d_0: np.ndarray
    approx_lambda_0d_1: np.ndarray
    approx_lambda_0d_2: np.ndarray
    approx_lambda_0d_3: np.ndarray

    # L2 relative errors for pressures, fluxes, and interface fluxes.
    error_p_2d_0: np.ndarray
    error_p_1d_0: np.ndarray
    error_p_1d_1: np.ndarray
    error_p_1d_2: np.ndarray
    error_p_1d_3: np.ndarray
    error_p_0d_0: np.ndarray
    error_u_2d_0: np.ndarray
    error_u_1d_0: np.ndarray
    error_u_1d_1: np.ndarray
    error_u_1d_2: np.ndarray
    error_u_1d_3: np.ndarray
    error_lambda_1d_0: np.ndarray
    error_lambda_1d_1: np.ndarray
    error_lambda_1d_2: np.ndarray
    error_lambda_1d_3: np.ndarray
    error_lambda_0d_0: np.ndarray
    error_lambda_0d_1: np.ndarray
    error_lambda_0d_3: np.ndarray
    error_lambda_0d_2: np.ndarray

    # Exact pressures, fluxes, and interface fluxes.
    exact_p_2d_0: np.ndarray
    exact_p_1d_0: np.ndarray
    exact_p_1d_1: np.ndarray
    exact_p_1d_2: np.ndarray
    exact_p_1d_3: np.ndarray
    exact_p_0d_0: np.ndarray
    exact_u_2d_0: np.ndarray
    exact_u_1d_0: np.ndarray
    exact_u_1d_1: np.ndarray
    exact_u_1d_2: np.ndarray
    exact_u_1d_3: np.ndarray
    exact_lambda_1d_0: np.ndarray
    exact_lambda_1d_1: np.ndarray
    exact_lambda_1d_2: np.ndarray
    exact_lambda_1d_3: np.ndarray
    exact_lambda_0d_0: np.ndarray
    exact_lambda_0d_1: np.ndarray
    exact_lambda_0d_3: np.ndarray
    exact_lambda_0d_2: np.ndarray
...
class DataSaving(VerificationDataSaving):
    ...
    def collect_data(self) -> SaveData:
        ...
        # Retrieve subdomains
        sd_2d_0: pp.Grid = self.mdg.subdomains()[0]
        sd_1d_0: pp.Grid = self.mdg.subdomains()[1]
        sd_1d_1: pp.Grid = self.mdg.subdomains()[2]
        sd_1d_2: pp.Grid = self.mdg.subdomains()[3]
        sd_1d_3: pp.Grid = self.mdg.subdomains()[4]
        sd_0d_0: pp.Grid = self.mdg.subdomains()[5]

        # Retrieve interfaces
        intf_1d_0: pp.MortarGrid = self.mdg.interfaces()[0]
        intf_1d_1: pp.MortarGrid = self.mdg.interfaces()[1]
        intf_1d_2: pp.MortarGrid = self.mdg.interfaces()[2]
        intf_1d_3: pp.MortarGrid = self.mdg.interfaces()[3]

        # Retrieve exact solution
        exact_sol: ExactSolution = self.exact_sol

        # Collect data
        exact_p_2d_0 = exact_sol.matrix_pressure(sd_2d_0)
    ...
    def bc_values_darcy(self, subdomains: list[pp.Grid]) -> pp.ad.DenseArray:
        ...
        for sd in subdomains:
            ...
            if sd.dim == self.mdg.dim_max():
                exact_bc = self.exact_sol.boundary_values()
...
class ManuIncompBalanceEquation(pp.fluid_mass_balance.MassBalanceEquations):
    ...
    def fluid_source(self, subdomains: list[pp.Grid]) -> pp.ad.Operator:
        ...
        for sd in subdomains:
            if sd.dim == self.mdg.dim_max():
                values.append(self.exact_sol.source(sd_matrix=sd))
            else:
