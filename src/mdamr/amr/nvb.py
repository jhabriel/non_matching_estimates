...
def refine_nvb(
    ...
    """Refine a mesh using the newest vertex bisection algorithm [1].

    Credits:
        This implementation is heavily based on [2], which is provided under MIT
        License. For the original source code, please refer to [3].

    Parameters:
        coordinates: np.ndarray
            Coordinates of the vertices. Shape is (num_vertices, 2). Rows are the
            vertices and columns are the x and y coordinates of each edge.
        elements: np.ndarray
            Map between triangles and vertices. Shape is (num_elements, 3). It is
            assumed that the vertices are given counterclockwise. Rows are the
            elements (cells) and columns are the vertices that define the triangle.
        marked_elements: np.ndarray, optional. Default is None.
            Boolean array of marked elements that should be refined. Shape is
            (num_elements, ). If not given, all elements in the grid will be refined.
            Note that this will correspond to a uniform refinement strategy.
        dirichlet_bc: np.ndarray, optional. Default is None.
            Array of boundary conditions of the Dirichlet type. Shape is
            (num_dirichlet_edges, 2). Rows correspond to a Dirichlet edge and columns to
            the vertices defining such an edge.
        neumann_bc: np.ndarray, optional. Default is None.
            Array of boundary conditions of the Neumann type. Shape is
            (num_neumann_edges, 2). Rows correspond to a Neumann edge and columns to the
            vertices defining such an edge.
        internal_bc: np.ndarray, optional. Default is None.
            Array of boundary conditions of the internal type (this type of boundary
            condition only make sense for mixed-dimensional grids). Shape is
            (num_internal_edges, 2). Rows correspond to an internal boundary edge and
            columns to the vertices defining such an edge.

    Returns:
        Five-tuple containing the following elements:
            - np.ndarray
                Coordinates of the vertices of the refined grid.
            - np.ndarray
                Mapping between triangles and vertices of the refined grid.
            - np.ndarray
                Array of boundary edges of the Dirichlet type of the refined grid.
            - np.ndarray
                Array of boundary edges of the Neumann type of the refined grid.
            - np.ndarray
                Array of boundary edges of the internal type of the refined grid.

    References:
        - [1] Original NVB papers
        - [2] Funken 2019 et al
        - [3] ameshref github repo
    ...
    # Get all marked elements. If not given, we assume uniform refinement
    ...
    # Establish hash patter for newest vertex bisection refinement
    ...
    new_elements[idx[red] - 1] = tmp[red][:, [3, 2, 5]]
    new_elements[idx[red]] = tmp[red][:, [0, 3, 5]]
    new_elements[idx[red] + 1] = tmp[red][:, [3, 1, 4]]
    new_elements[idx[red] + 2] = tmp[red][:, [2, 3, 4]]
